...
void TabEvents::listEvents_currentRowChanged(int row) {
  ...
  if(auto i = ui->cmbEventParam->findText(model.getParameters().getName(events.getVariable(currentEventId))); i>=0){
...
void TabEvents::btnAddEvent_clicked() {
  if(model.getParameters().getIds().isEmpty()){
    SPDLOG_WARN("No parameters to set via events");
    return;
  }
  bool ok{false};
  ...
  if (ok && !eventName.isEmpty()) {
    auto newEventName{model.getEvents().add(eventName, model.getParameters().getIds()[0])};
...
void TabEvents::cmbEventParam_activated(int index) {
  ...
  if(i < model.getParameters().getIds().size()){
    model.getEvents().setVariable(currentEventId, model.getParameters().getIds()[i]);
  }
}
