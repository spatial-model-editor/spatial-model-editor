...
void ModelEvents::setVariable(const QString &id, const QString &variable) {
  SPDLOG_INFO("Setting event '{}' variable to '{}'", id.toStdString(),
  ...
  hasUnsavedChanges = true;
  event->getEventAssignment(0)->setVariable(variable.toStdString());
}
...
void ModelEvents::setTime(const QString &id, double time) {
  auto triggerExpression{fmt::format("t=={}", time)};
...
double ModelEvents::getTime(const QString &id) const {
  ...
  auto *ast{event->getTrigger()->getMath()};
  if (auto *rhs{ast->getRightChild()}; rhs->isNumber()) {
    return rhs->getValue();
  }
  if (auto *lhs{ast->getLeftChild()}; lhs->isNumber()) {
    return lhs->getValue();
  }
  SPDLOG_WARN("Trigger '{}' is not of supported form 't == 1'",
              mathASTtoString(ast));
  return 0;
}
...
QString ModelEvents::add(const QString &name, const QString &variable) {
  const std::string defaultTrigger{"t == 0"};
  ...
  SPDLOG_INFO("  - Trigger: {}", defaultTrigger);
  event->setUseValuesFromTriggerTime(false);
  auto *trigger{event->createTrigger()};
  trigger->setInitialValue(false);
  trigger->setPersistent(false);
  trigger->setMath(mathStringToAST(defaultTrigger, sbmlModel).get());
  SPDLOG_INFO("  - Variable: {}", eventVariable);
  SPDLOG_INFO("  - Value: {}", defaultValue);
  auto *asgn{event->createEventAssignment()};
  asgn->setVariable(eventVariable);
  asgn->setMath(mathStringToAST(defaultValue, sbmlModel).get());
