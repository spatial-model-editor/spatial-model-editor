...

{
  ...
  if (status != 0) {
    throw std::runtime_error("Triangle library failed to triangulate mesh.");
  }
  exit(status);
}
...

{
  ...
  pool->nextitem = (VOID *)(alignptr + (unsigned long long)pool->alignbytes -
...

{
  ...
  if (pool->deaditemstack != (VOID *)NULL) {
    ...
    if (pool->unallocateditems == 0) {
      ...
      /* Align the item on an `alignbytes'-byte boundary. */
      pool->nextitem =
          (VOID *)(alignptr + (unsigned long long)pool->alignbytes -
                   (alignptr % (unsigned long long)pool->alignbytes));
      /* There are lots of unallocated items left in this block. */
...

{
  ...
  pool->pathitem = (VOID *)(alignptr + (unsigned long long)pool->alignbytes -
                            (alignptr % (unsigned long long)pool->alignbytes));
...
#else  /* not ANSI_DECLARATORS */
unsigned long long randomnation(choices) unsigned int choices;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
long long removeghosts(struct mesh *m, struct behavior *b,
                       struct otri *startghost)
#else  /* not ANSI_DECLARATORS */
long long removeghosts(m, b, startghost) struct mesh *m;
struct behavior *b;
struct otri *startghost;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
long long divconqdelaunay(m, b) struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
long long removebox(m, b) struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
long long incrementaldelaunay(m, b) struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
long long sweeplinedelaunay(m, b) struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
long long delaunay(m, b) struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
long long reconstruct(m, b, elefilename, areafilename, polyfilename,
...

{
  ...
  if (!b->quiet && b->conformdel && (m->badsubsegs.items > 0) &&
    ...
    if (m->badsubsegs.items == 1) {
      ...
      printf(
          "  %lld encroached subsegments, and therefore might not be truly\n",
          m->badsubsegs.items);
    }
