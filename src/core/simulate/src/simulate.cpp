...
void Simulation::applyNextEvent() {
  ...
  if (simulatorType == SimulatorType::DUNE &&
    ...
    simulator = std::make_unique<DuneSim>(
        *nextModel.get(), compartmentIds, compartmentSpeciesIds, simulatorOptions.dune);
  } else {
    simulator = std::make_unique<PixelSim>(
        *nextModel.get(), compartmentIds, compartmentSpeciesIds, simulatorOptions.pixel);
  }
  // remove event time
  eventTimes.pop();
}
...
std::size_t Simulation::doTimesteps(double time, std::size_t nSteps,
  ...
  for (std::size_t iStep = 0; iStep < nSteps; ++iStep) {
    ...
    if (timePoints.back() + time > eventTimes.front()) {
      // event occurs during step: need to split the step in two & apply event in between
