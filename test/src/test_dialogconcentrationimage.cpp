...
#ifndef Q_OS_MACOS
SCENARIO("DialogConcentrationImage", "[dialogconcentrationimage][gui]") {
  GIVEN("3x3 compartment image, 4 pixel compartment") {
    ...
    sbml::SpeciesGeometry specGeom{sz, compartmentPoints, origin, width,
                                   doc.getModelUnits()};
    ...
    WHEN("all-black pixel image loaded") {
      ...
      mwtImage.addUserAction("tmp.png");
      mwt.start();
      dia.exec();
      auto a = dia.getConcentrationArray();
      for (auto i : compArrayIndices) {
