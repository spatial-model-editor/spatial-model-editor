...
TEST_CASE("reactions for ABtoC model", "[reactions][non-gui]") {
  ...
  REQUIRE(reac.constants[0].at("comp") == dbl_approx(3149.0));
  REQUIRE(reac.constants[0].at("k1") == dbl_approx(0.1));
  REQUIRE(reac.M[0][0] == dbl_approx(-3.1756113051762465e-04));
  REQUIRE(reac.M[0][1] == dbl_approx(-3.1756113051762465e-04));
  REQUIRE(reac.M[0][2] == dbl_approx(+3.1756113051762465e-04));
}
