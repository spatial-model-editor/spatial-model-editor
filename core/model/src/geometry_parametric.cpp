...
GeometryParametricImportResult importFixedMeshFromParametricGeometry(
  ...
  if (nCoords != 3) {
    result.diagnostic = QString("Unsupported ParametricGeometry dimensions: %1 "
                                "(only 3D fixed-mesh import is supported)")
                            .arg(nCoords);
    return result;
  }
  ...
  if (pointData.size() < 3 || pointData.size() % 3 != 0) {
    result.diagnostic = QString("ParametricGeometry spatialPoints length (%1) "
                                "is invalid for 3D coordinates")
                            .arg(pointData.size());
    return result;
  }
  ...
  for (unsigned i = 0; i < pg->getNumParametricObjects(); ++i) {
    ...
    const auto importTriangleSurfaceAsTetraFan = [&]() -> bool {
      ...
      for (std::size_t k = 0; k < pointIndex.size(); k += 3) {
        ...
        objectVertices.insert(face[0]);
        objectVertices.insert(face[1]);
        objectVertices.insert(face[2]);
      }
      ...
      for (const auto &face : uniqueFaces) {
        imported.mesh.tetrahedra.push_back(
            {{{face[0], face[1], face[2], centroidIndex}}, physicalTag});
      }
...
GeometryParametricExportResult exportFixedMeshToParametricGeometry(
  ...
  QStringList skippedCompartmentsNoDomainType;
  std::size_t nExportedObjects{0};
  for (std::size_t iComp = 0; iComp < tetrahedra.size(); ++iComp) {
    if (iComp >= static_cast<std::size_t>(compartmentIds.size()) ||
        iComp >= static_cast<std::size_t>(compartmentColors.size()) ||
        tetrahedra[iComp].empty() ||
    ...
    if (domainIt == compartmentIdToDomainType.cend()) {
      skippedCompartmentsNoDomainType.push_back(compartmentId.c_str());
      continue;
    }
  ...
  if (nExportedObjects == 0) {
    if (skippedCompartmentsNoDomainType.isEmpty()) {
      result.diagnostic =
          "Failed to export fixed mesh: no tetrahedra found for "
          "exportable compartments";
    } else {
      result.diagnostic =
          QString("Failed to export fixed mesh: missing domainType mapping for "
                  "compartments %1")
              .arg(skippedCompartmentsNoDomainType.join(", "));
    }
    return result;
  }
  ...
  if (!skippedCompartmentsNoDomainType.isEmpty()) {
    result.diagnostic =
        QString("Fixed mesh export skipped compartments without domainType "
                "mapping: %1")
            .arg(skippedCompartmentsNoDomainType.join(", "));
  }
  return result;
