...
// a struct that describes 3d segment, Point is the data structure of vertex of
// segment.
template <typename Point> struct mySegment3d {
  Point begin;
  Point end;
  mySegment3d() {}
...
// Segs3 is an array of mySegment3d which can be traveled by iterator, such as
// std::veector<mySegment3d>.
...
public:
  ...
  SimpleSegments3ViewerQt(QWidget *parent, const Segs3 &seg3,
                          const char *title = "Basic Segs3 Viewer")
      : // First draw: vertices, edges, faces, multi-color, no inverse normal
        Base(parent, title, true, true, false, false, true), s3(seg3) {
    compute_elements();
  }
...
void draw(const Segs3 &s3, const char *title = "Segs3 Basic Viewer") {
  ...
  if (!cgal_test_suite) {
    ...
    SimpleSegments3ViewerQt<Segs3, Point> mainwindow(app.activeWindow(), s3,
                                                     title);
    mainwindow.show();
    app.exec();
  }
