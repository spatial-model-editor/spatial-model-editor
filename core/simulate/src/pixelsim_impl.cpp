...
std::string SimCompartment::plotRKError(std::vector<QImage> &images,
                                        double epsilon, double max) const {
  auto imageSize{comp->getImageSize()};
  images = {imageSize.depth(), QImage(imageSize.width(), imageSize.height(),
                                      QImage::Format_RGB32)};
...
const std::vector<geometry::Voxel> &SimCompartment::getVoxels() const {
